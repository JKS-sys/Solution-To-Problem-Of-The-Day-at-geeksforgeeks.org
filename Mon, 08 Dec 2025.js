...
// Difficulty: Hard Accuracy: 59.66% Submissions: 35K+ Points: 8 Average Time: 25m
...
class Solution {
  matrixChainOrder(arr) {
    ...
    if (matrices === 1) return "A";

    const dp = Array.from({ length: matrices }, () =>
      new Array(matrices).fill(0)
    );
    const bracket = Array.from({ length: matrices }, () =>
      new Array(matrices).fill(0)
    );

    for (let length = 2; length <= matrices; length++) {
      // length of chain
    ...
    return this.buildString(bracket, 0, matrices - 1);
  }

  buildString(bracket, i, j) {
    if (i === j) {
      return String.fromCharCode("A".charCodeAt(0) + i);
    }

    const k = bracket[i][j];
    const left = this.buildString(bracket, i, k);
    const right = this.buildString(bracket, k + 1, j);
